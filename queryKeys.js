...
export function returnSentenceCasedString(stringValue) {
...
async function updateValues(endpointIndex) {
    ...
    try {
      ...
      if (count > 0) {
        ...
        if (endpoints[endpointIndex] === "inventoryEntries") {
          const updateCalls = queryResults[0][
  ...
  console.log(
    `Finished applying keys to ${returnSentenceCasedString(
      endpoints[endpointIndex]
    )}.`
  );
}
...
//   console.log(
//     `${
//       processedResults[endpoints[answersIndex[i]]]
//     } ${returnSentenceCasedString(endpoints[answersIndex[i]])} need keys.`
//   );
//   if (processedResults[endpoints[answersIndex[i]]] > 0) {
//     keysNeededForTheseEndpoints.push(i);
//   }
// }
...
//       choices: keysNeededForTheseEndpoints.map((index) =>
//         returnSentenceCasedString(endpoints[index])
//       ),
//     },
//   ]);

//   // Create an array of endpoint indexes.
//   const endpointsToUpdateIndex = Array.isArray(updateAnswers.endpointsToUpdate)
//     ? updateAnswers.endpointsToUpdate.map((choice) =>
//         endpoints.map(returnSentenceCasedString).indexOf(choice)
//       )
//     : [
//         endpoints
//           .map(returnSentenceCasedString)
//           .indexOf(updateAnswers.endpointsToUpdate),
//       ];

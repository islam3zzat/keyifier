...
async function updateValues(isAssets) {
    ...
    do {
        try {
            ...
            if (count > 0) {
                ...
                if (isAssets) {
                    const updateCalls = queryResults[0].categories.results.map(({ id, version, assets }) => {
                    ...
                    const updateCalls = queryResults[0].categories.results.map(result => ({
                        url: `${CTP_API_URL}/${CTP_PROJECT_KEY}/graphql`,
                        options: {
                            method: 'POST',
                            headers: {
    ...
    isAssets ? console.log(`Finished applying keys to Category Assets.`) : console.log(`Finished applying keys to Categories.`);
}
...
const answersIndex = answers.selectedResource.map(choice => resourceTypes.indexOf(choice));

let whereQueries = [];
if (answers.selectedResource.includes("Categories")) { whereQueries.push(whereCategory); }
if (answers.selectedResource.includes("Category Assets")) { whereQueries.push(whereCategoryAsset); }
...
for (let i = 0; i < answersIndex.length; i++) {
    ...
    if (answersIndex[i] === 1) {
        console.log(`${processedResults["1"]} Categories have Assets that need keys.`)
...
if (keysNeededForTheseResources.length > 0) {
    ...
    const updateAnswers = await inquirer.prompt([
        {
            type: keysNeededForTheseResources.length === 1 ? "select" : 'checkbox',
            name: 'resourcesToUpdate',
            message: 'Select the resources to apply keys to:',
            choices: keysNeededForTheseResources.map(index => resourceTypes[index])
        }
    ]);

    const resourcesToUpdateIndex = Array.isArray(updateAnswers.resourcesToUpdate) ? updateAnswers.resourcesToUpdate.map(choice => resourceTypes.indexOf(choice)) : [resourceTypes.indexOf(updateAnswers.resourcesToUpdate)];

...
export async function applyProductKeys() {
  ...
  if (total > 0) {
    ...
    do {
      try {
        ...
        // Update keys
        if (count > 0) {
          const updateCalls = queryResults[0].products.results.map(
            (result) => ({
              url: `${CTP_API_URL}/${CTP_PROJECT_KEY}/graphql`,
              options: {
                method: "POST",
...
export async function applyProductVariantKeys() {
    ...
    try {
      ...
      if (count > 0) {
        for (let i = 0; i < queryResults[0].products.results.length; i++) {
          ...
          let newMasterKey =
            queryResults[0].products.results[i].masterData.current.masterVariant
              .key;

          // If not set, use the staged
          if (newMasterKey === null) {
            newMasterKey =
              queryResults[0].products.results[i].masterData.staged
                .masterVariant.key;
          }
          ...
          if (newMasterKey === null) {
            newMasterKey =
              queryResults[0].products.results[i].masterData.current
                .masterVariant.sku === null
                ? queryResults[0].products.results[i].masterData.staged
                    .masterVariant.sku
                : queryResults[0].products.results[i].masterData.current
                    .masterVariant.sku;
          ...
          updates[
            i
          ] = `{ setProductVariantKey: { variantId: ${queryResults[0].products.results[i].masterData.current.masterVariant.id}, key: "${newMasterKey}" staged: false } }`;
        }
    ...
    try {
      ...
      if (count > 0) {
        for (let i = 0; i < queryResults[0].products.results.length; i++) {
          ...
          if (
            queryResults[0].products.results[i].masterData.current.variants
              .length ===
            queryResults[0].products.results[i].masterData.current.variants
              .length
          ) {
            for (
              ...
              if (newVariantKey === null) {
                ...
                updates[
                  i
                ] += ` { setProductVariantKey: { variantId: ${queryResults[0].products.results[i].masterData.current.variants[ii].id}, key: "${newVariantKey}" staged: false } }`;
              }
            }
          } else {
            console.log(
              `Product: ${queryResults[0].products.results[i].id}. The number of Variants differs between current and staged projections. You must update their keys manually.`
            );
          }
        }
...
export async function applyProductPriceKeys() {
          ...
          if (
            ...
            if (
              ...
              for (
                ...
                let indexOfStagedPrice = getArrayIndex(
                  queryResults[0].products.results[i].masterData.staged
                    .masterVariant.prices,
                  queryResults[0].products.results[i].masterData.current
                    .masterVariant.prices[ii].id
                );
                ...
                let newMasterPriceKey =
                  queryResults[0].products.results[i].masterData.current
                    .masterVariant.prices[ii].key;

                if (newMasterPriceKey === null) {
                  newMasterPriceKey =
                    queryResults[0].products.results[i].masterData.staged
                      .masterVariant.prices[indexOfStagedPrice].key;
                }
                ...
                updates[
                  i
                ] += ` { setPriceKey: { priceId: "${queryResults[0].products.results[i].masterData.current.masterVariant.prices[ii].id}", key: "${newMasterPriceKey}" staged: false } }`;
              }
            }
          } else {
            console.log(
              `Product: ${queryResults[0].products.results[i].id}. The number of Master Variant Prices differs between current and staged projections. You must update their Price keys manually.`
            );
          }
        }
      ...
      if (count > 0) {
        ...
        for (let i = 0; i < queryResults[0].products.results.length; i++) {
          ...
          for (
            ...
            if (
              queryResults[0].products.results[i].masterData.current.variants[v]
                .prices.length > 0
            ) {
              // Only update the price keys if they're identical
              if (
                ...
                queryResults[0].products.results[i].masterData.staged.variants[
                  v
                ].prices.length
              ) {
                for (
                  ...
                  let newPriceKey =
                    queryResults[0].products.results[i].masterData.current
                      .variants[v].prices[ii].key;

                  if (newPriceKey === null) {
                    newPriceKey =
                      queryResults[0].products.results[i].masterData.staged
                        .variants[v].prices[indexOfStagedPrice].key;
                  }
                  ...
                  updates[
                    i
                  ] += ` { setPriceKey: { priceId: "${queryResults[0].products.results[i].masterData.current.variants[v].prices[ii].id}", key: "${newPriceKey}" staged: false } }`;
                }
              }
            } else {
              console.log(
                `Product: ${queryResults[0].products.results[i].id}. The number of Variant Prices differs between current and staged projections. You must update their Price keys manually.`
              );
            }
          }
        }
...
export async function applyProductAssetKeys() {
          ...
          if (
            ...
            if (
              ...
              for (
                ...
                let indexOfStagedAsset = getArrayIndex(
                  queryResults[0].products.results[i].masterData.staged
                    .masterVariant.assets,
                  queryResults[0].products.results[i].masterData.current
                    .masterVariant.assets[ii].id
                );
                ...
                let newMasterAssetKey =
                  queryResults[0].products.results[i].masterData.current
                    .masterVariant.assets[ii].key;

                if (newMasterAssetKey === null) {
                  newMasterAssetKey =
                    queryResults[0].products.results[i].masterData.staged
                      .masterVariant.assets[indexOfStagedAsset].key;
                }
                ...
                updates[
                  i
                ] += ` { setAssetKey: { assetId: "${queryResults[0].products.results[i].masterData.current.masterVariant.assets[ii].id}", assetKey: "${newMasterAssetKey}", variantId: ${queryResults[0].products.results[i].masterData.current.masterVariant.id}, staged: false } }`;
              }
            }
          } else {
            console.log(
              `Product: ${queryResults[0].products.results[i].id}. The number of Master Variant Assets differs between current and staged projections. You must update their Asset keys manually.`
            );
          }
        }
            ...
            if (
              ...
              if (
                ...
                for (
                  ...
                  let newAssetKey =
                    queryResults[0].products.results[i].masterData.current
                      .variants[v].assets[ii].key;

                  if (newAssetKey === null) {
                    newAssetKey =
                      queryResults[0].products.results[i].masterData.staged
                        .variants[v].assets[indexOfStagedAsset].key;
                  }
                  ...
                  updates[
                    i
                  ] += ` { setAssetKey: { assetId: "${queryResults[0].products.results[i].masterData.current.variants[v].assets[ii].id}", assetKey: "${newAssetKey}", variantId: ${queryResults[0].products.results[i].masterData.current.variants[v].id},  staged: false } }`;
                }
              } else {
                console.log(
                  `Product: ${queryResults[0].products.results[i].id}. The number of Variant Assets differs between current and staged projections. You must update their Asset keys manually.`
                );
              }
            }
          }
